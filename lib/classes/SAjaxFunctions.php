...
class SAjaxFunctions extends SAjax
{
	...
	public function cookieAlert($class) {
        setcookie("cookieAlert", "accepted", strtotime( '+30 days' ), '/');     
        $this->out('script', '$("#'.$class.'").hide();');
    }
	
	public function checkCaptcha($r) {
		$this->out('script','$("#error_frame").html("");');
		$secretKey = _GOOGLE_SECRET_KEY;

		$ip = $_SERVER['REMOTE_ADDR'];
	    $response=file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=".$secretKey."&response=".$r."&remoteip=".$ip);
		$responseKeys = json_decode($response,true);
		if(intval($responseKeys["success"]) !== 1) {
		    //  response NOT OK 
		    $status = 'not-ok';
		    $this->out('script','$("#error_frame").html("Security");grecaptcha.reset();');
		}else{
			$status = 'ok';
			$this->out('script','$("#g-response").val("ok")');
			$this->out('script','document.getElementById("f_forma").submit();');
		}
...
public function favorits($id,$table,$lng)
	...
	public function comment($data){
		...
		if( get_conf('multi_language') == 1 )
			load_lang();
		$req = array('ime_comm','message_comm');
