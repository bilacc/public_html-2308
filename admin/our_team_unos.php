...
?>
	...
	{
	?>
		<h1>Uređivanje članova <br/><a href="<?php echo $crud->table; ?>_unos.php" class="gumb">Dodaj novog</a></h1>
	<?php
	}else{
	?>
		<h1>Unos članova</h1>
	...
	<form name="<?php echo $crud->table; ?>" id="<?php echo $crud->table; ?>" class="unos" action="" enctype="multipart/form-data" method="post">
			...
			<div class="tabs-title-content">
				...
				?>
					<div class="editor-title slc" id="tab_title_hr_content">
						...
						<input type="text" placeholder="Titula" name="titula" id="titlula" value="<?php echo $data['titula']; ?>"/>
						
						<input type="text" placeholder="Područje djelovanja" name="area" id="area" value="<?php echo $data['area']; ?>"/>
					</div>
				<?php }else{
					for($i = 0; $i < sizeof($column_name); $i++)
					{
		...
		<div class="box-25 light">
			<input type="text" placeholder="Telefon" name="tel" id="tel" value="<?php echo $data['tel']; ?>"/>
			<input type="text" placeholder="E mail" name="mail" id="mail" value="<?php echo $data['mail']; ?>"/>
		</div>
		<?php
			if($show['url'])
			{
