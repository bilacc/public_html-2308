<?php 
	...
	$crud->return_url = 'items_uvod.php';
	...
	$show['featured_img2'] = true; 	
	$show['featured_img3'] = true; 		//izdvojena slika (bivši static image, ali se drugačije sprema)		
			...
			<div class="tabs-title-content">
				...
				?>
					<div class="editor-title slc" id="tab_title_hr_content">
						<input type="text" placeholder="Naslov stanovi Zagreb" name="title_hr" id="title_hr" value="<?php echo $data['title_hr']; ?>"/>
						<input type="text" placeholder="Podnaslov" name="subtitle_hr" id="subtitle_hr" value="<?php echo $data['subtitle_hr']; ?>"/>
					</div>
				<?php }else{
					for($i = 0; $i < sizeof($column_name); $i++)
					{
		...
		?>
			<div class="tabs-title-content">
					...
					<div class="editor-title slc" id="tab_title_hr_content">
						...
						<input type="text" placeholder="Naslov - Ekskluzivni stanovi" name="title4_hr" id="title4_hr" value="<?php echo $data['title4_hr']; ?>"/>
						<input type="text" placeholder="Podnaslov - Ekskluzivni stanovi" name="subtitle4_hr" id="subtitle4_hr" value="<?php echo $data['subtitle4_hr']; ?>"/>
				</div>
			</div>	
			...
			?>
				...
				if(sizeof($column_name) == 1){
				?>		
					<a href="javascript:;" id="tab_hr" class="slc">Uvodni tekst - <strong>stanovi Zagreb</strong></a>
					<a href="javascript:;" id="tab2_hr" class="">Uvodni tekst - <strong>Ekskluzivne nekretnine</strong></a>
				<?php
				}else{
					for($i = 0; $i < sizeof($column_name); $i++)
					{
					?>
					<a href="javascript:;" id="tab_<?php echo strtolower($lang_label[$i]);?>" <?php echo ($i == 0)? 'class="slc"':''; ?>>Uvodni tekst <span>(<?php echo $lang_label[$i];?>)</span></a>
				<?php
					}
				}
				?>
				...
				?>		
					...
					<div class="editor" id="tab2_hr_content">
						<textarea class="ckeditor" name="text4_hr"><?php echo isset($data['text4_hr'])? $data['text4_hr'] : '' ; ?></textarea>
					</div>
				<?php
				}else{
					for($i = 0; $i < sizeof($column_name); $i++)
			...
			<div class="box-100 light">	
				...
				?>
					...
					?>
						<a href="javascript:;" onclick="sjx('open_img_selection2', '<?php echo $crud->table; ?>', <?php echo $id; ?>);return false;" class="set_featured_img">Postavi izdvojenu sliku</a>
					<?php }else{ ?>
						<div class="no_entry" style="font-size:14px;">Stranicu prvo morate spremiti kako bi mogli postaviti izdvojenu sliku.</div>
					<?php } ?>
							...
							<div id="featured_holder2" class="featured-holder">
								...
								?>
									...
									<img class="featured_img2" style="width:100%;" src="../upload_data/site_photos/th_<?php echo $featured_img2; ?>" alt="" />
								<?php } ?>
							</div>
				...
				<div class="box-50 last">
					<div class="section active-section" id="odlomak_1_hr_container">
						<label for="title2_hr">Naslov</label>
						<input type="text" name="title2_hr" id="title2_hr" value="<?php echo $data['title2_hr']; ?>"/>
					
						<label for="subtitle2_hr">Podnaslov</label>
						<input type="text" name="subtitle2_hr" id="subtitle2_hr" value="<?php echo $data['subtitle2_hr']; ?>"/>
					</div>
				</div>
				<div class="box-100">
			...
			<div class="box-100 light">	
				...
				<div class="box-50 ">
					<h3>Izdvojena slika</h3>
					<?php
						if($show['featured_img3']){
						$featured_img3 = Db::query_one('SELECT photo_name FROM site_photos WHERE id = '.$data['image3']);
					?>

					<?php
						if( isset($_GET['action']) && isset($_GET['id']) && ! $_POST ){
					?>
						<a href="javascript:;" onclick="sjx('open_img_selection3', '<?php echo $crud->table; ?>', <?php echo $id; ?>);return false;" class="set_featured_img">Postavi izdvojenu sliku</a>
					<?php }else{ ?>
						<div class="no_entry" style="font-size:14px;">Stranicu prvo morate spremiti kako bi mogli postaviti istaknutu sliku.
						</div>
						<?php } ?>
						<div id="featured_holder3" class="featured-holder">
							<?php
								if($featured_img3){
							?>
								<a href="javascript:;" onclick="$('#image3').val('0');$('#featured_holder3').empty();" class="del_img"><img src="images/icon-delete-round.png" alt="Briši" /></a>
								<img class="featured_img3" style="width:100%;" src="../upload_data/site_photos/th_<?php echo $featured_img3; ?>" alt="" />
							<?php } ?>
						</div>
						<input type="hidden" name="image3" id="image3" value="<?php echo $data['image3']; ?>" />
					<?php
						}
					?>
				</div>
				<div class="box-50 last">
					...
					<div class="section2 active-section" id="odlomak_2_hr_container">
						<label for="title3_hr">Naslov</label>
						<input type="text" name="title3_hr" id="title3_hr" value="<?php echo $data['title3_hr']; ?>"/>
						<label for="subtitle3_hr">Podnaslov</label>
						<input type="text" name="subtitle3_hr" id="subtitle3_hr" value="<?php echo $data['subtitle3_hr']; ?>"/>
					</div>
				</div>
				<div class="box-100">
