<?php 
	...
	$show['multiple_category'] = true; //mogućnost pridodavanja stranice ili članka u više kategorija (imploda array u string i sprema u multi_categories)
	...
	$cat = new Admin_ManageCategories($data['id'],'categories_usluge', $crud->table);
	$cat2 = new Admin_ManageCategories($data['id'],'services', $crud->table);
...
?>
	...
	{
	?>
		<h1>Uređivanje usluga <br/><a href="<?php echo $crud->table; ?>_unos.php" class="gumb">Dodaj novu</a></h1>
	<?php
	}else{
	?>
		<h1>Unos usluga</h1>
	...
	<form name="<?php echo $crud->table; ?>" id="<?php echo $crud->table; ?>" class="unos" action="" enctype="multipart/form-data" method="post">
		...
		<?php
			...
			?>
				...
				<?php
					...
					{
					?>
					<a href="javascript:;" id="tab2_<?php echo strtolower($lang_label[$i]);?>" <?php echo ($i == 0)? 'class="slc"':''; ?>>Uvodni Tekst <span>(<?php echo $lang_label[$i];?>)</span></a>
					<a href="javascript:;" id="tab_<?php echo strtolower($lang_label[$i]);?>" <?php echo ($i == 0)? 'class=""':''; ?>>Tekst <span>(<?php echo $lang_label[$i];?>)</span></a>
				<?php
					}
				}
				?>
				...
				<?php
					...
					{
					?>
					<div class="editor " id="tab_<?php echo strtolower($lang_label[$i]);?>_content">
					...
					<div class="editor <?php echo ($i == 0)? '':''; ?><?php echo ($i == 0)? 'slc':''; ?>" id="tab2_<?php echo strtolower($lang_label[$i]);?>_content">
						<textarea class="ckeditor" name="subtitle_<?php echo strtolower($lang_label[$i]);?>"><?php echo isset($data['subtitle_'.strtolower($lang_label[$i])])? $data['subtitle_'.strtolower($lang_label[$i])] : '' ; ?></textarea>
					</div>
				<?php
					}
				}
		...
		<div class="box-25 last light">
			...
			<select name="categories_id" id="categories_id">
				...
				<?php echo $cat->display_tree_select(0,0,$data['categories_id'],1); ?>
			</select> 
			<?php
			}
			?>			
			...
			<h4>Glavna usluga:</h4>
				...
				<?php 
			$services= Db::query("SELECT * FROM services ORDER BY orderby ASC");
			?>

			<select name="usluga_id" id="usluga_id">
				<option value=""> -Nije dodano uslugi- </option>
				<?php foreach ($services as $red) {
					$cat_ser= Db::query_row("SELECT * FROM categories_usluge WHERE id=".$red['categories_id']." ORDER BY orderby ASC");
				?>
					<option value="<?php echo $red['id']?>" <?php echo ($data['usluga_id'] == $red['id'])? 'selected="selected"':'';?>>
						<?php echo $red['title_hr'];?> - <?php echo $cat_ser['title_hr'];?>
					</option>
					<?php 
						foreach ($sub_services as $sub_red) {
						$subsub_services= Db::query("SELECT * FROM services WHERE parent_id=".$sub_red['id']." ORDER BY orderby ASC");
					?>
						<option value="<?php echo $sub_red['id']?>" <?php echo ($data['usluga_id'] == $sub_red['id'])? 'selected="selected"':'';?>>
						  &nbsp;&nbsp;-<?php echo $sub_red['title_hr'];?>
						</option>
						<?php 
							foreach ($subsub_services as $subsub_red) {
							$subsubsub_services= Db::query("SELECT * FROM services WHERE parent_id=".$subsub_red['id']." ORDER BY orderby ASC");
						?>
							<option value="<?php echo $subsub_red['id']?>" <?php echo ($data['usluga_id'] == $subsub_red['id'])? 'selected="selected"':'';?>>
						   	&nbsp;&nbsp;&nbsp;&nbsp;--<?php echo $subsub_red['title_hr'];?>
							</option>

							<?php 
								foreach ($subsubsub_services as $subsubsub_red) {
								$subsubsubsub_services= Db::query("SELECT * FROM services WHERE parent_id=".$subsubsub_red['id']." ORDER BY orderby ASC");
							?>
							<option value="<?php echo $subsubsub_red['id']?>" <?php echo ($data['usluga_id'] == $subsubsub_red['id'])? 'selected="selected"':'';?>>
			...
			$kategorije = Db::query('SELECT title_hr, id FROM services ORDER BY orderby ASC');
			...
			?>
				...
				<label for="cat-0">Nije dodan članak</label>
				...
				?>
					...
					<label for="cat-<?php echo $kat['id']; ?>"><?php echo $kat['title_hr']; ?> - <?php echo $cat_ser['title_hr'];?></label>
					<div class="clearfix"></div>	
				<?php 
				} 
			}
		...
		?>
			...
			<h3>Prikazi slike na detaljima</h3>
				<select name="prikazi" id="prikazi">
				<option value="da" <?php echo ($data['prikazi'] == 'da')? 'selected="selected"':'';?>>Da</option>
				<option value="ne" <?php echo ($data['prikazi'] == 'ne')? 'selected="selected"':'';?>>Ne</option>
