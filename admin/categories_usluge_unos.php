...
?>
	...
	{
	?>
		<h1>Uređivanje kategorije <a href="<?php echo $table; ?>_unos.php" class="gumb">Dodaj novu</a></h1>
	<?php
	}else{
	?>
	...
	<form name="<?php echo $crud->table; ?>" id="<?php echo $crud->table; ?>" class="unos" action="" enctype="multipart/form-data" method="post">
		...
		?>
			...
			?>
				...
				if(sizeof($column_name) == 1){
				?>		
					<a href="javascript:;" id="tab2_hr">Kratki tekst</a>
...
<?php
		...
		<div class="box-25 last light">
			...
			if($show['new']){
			?>
			<h4>Izdvojeno u NOVO</h4>
			
			<select name="new" id="new">
				<option value="ne" <?php echo ($data['new'] == 'ne')? 'selected="selected"':'';?>>Ne</option>
				<option value="da" <?php echo ($data['new'] == 'da')? 'selected="selected"':'';?>>Da</option>
			</select>
			<?php
			}
			?>
