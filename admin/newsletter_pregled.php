<?php 
	...
	if(!file_exists(_SITE_ROOT.'include/newsletter.php'))
		echo '<div class="error">Nedostaje newsletter.php dokument na front-end djelu u folderu "include" !!!</div>'
?>
	<div class="success" id="nl-success" style="display:none;">Uspješno ste poslali newsletter.</div>
	<div class="error" id="nl-error" style="display:none;">Newsletter nije uspješno poslan. Molimo pokušajte ponovno.</div>

	<h1>Newsletter <br/><a href="<?php echo $table; ?>_unos.php" class="gumb">Dodaj novi</a></h1>
					...
					<tr id="row_<?php echo $v['id']; ?>">
						<td class="naslov">
							...
							<div class="opcije">
								...
								<a href="javascript:;" title="Pošalji newsletter" class="send" onclick="if(confirm('Jeste li sigurni da želite poslati newsletter?')){ sjx('send_newsletter','real',<?php echo $v['id']; ?>); return false; }"><img src="images/icon-send.png" alt="Pošalji newsletter" /></a>
								<a href="javascript:;" title="Pošalji testni newsletter" class="test" onclick="if(confirm('Jeste li sigurni da želite poslati testni newsletter?')){ sjx('send_newsletter','test',<?php echo $v['id']; ?>); return false; }"><img src="images/icon-test.png" alt="Pošalji testni" /></a>
							</div>
						</td>
