...
if(_su2 != '_su2' && !(is_numeric(_su2))){
	...
	foreach ($sub_lokacija as $red1) {
		$items1 = Db::query_one('SELECT id FROM items WHERE najam="da" AND city_id = '.$red1['id'].' AND aktivno ="da"');
		...
		if($sub_sub_lokacija){
			foreach ($sub_sub_lokacija as $red2) {
				$items2 = Db::query_one('SELECT id FROM items WHERE najam="da" AND city_id = '.$red2['id'].' AND aktivno ="da"');
				if($items2){
	...
	if(isset($_GET['location'])){
		...
		$all_items = Db::query('SELECT id FROM items WHERE aktivno ="da" AND najam ="da" AND 1'.$sql_location);	
		$page = (is_numeric(_su3)) ? _su3 : 1;
		$start = $on_page * ($page-1);
		$items = Db::query('SELECT * FROM items WHERE 1'.$sql_location.' AND najam ="da" AND aktivno = "da" ORDER BY orderby DESC LIMIT '.$start.', '.$on_page);
		$ukupno = count($all_items);
		$stranica = ceil($ukupno/$on_page);
	}else{
		$all_items = Db::query('SELECT id FROM items WHERE najam ="da" AND aktivno ="da" AND city_id IN('.$lokacije_idijevi.') AND 1'.$sql_location);
		$page = (is_numeric(_su3)) ? _su3 : 1;
		$start = $on_page * ($page-1);
		$items = Db::query('SELECT * FROM items WHERE najam ="da" AND city_id IN('.$lokacije_idijevi.') AND aktivno = "da" ORDER BY orderby DESC LIMIT '.$start.', '.$on_page);
		$ukupno = count($all_items);
		$stranica = ceil($ukupno/$on_page);
	}
	$kat_filter = clean_uri($main['title_'._LNG]).'-'.$main_id;
...
<div class="bc row">
    <div class="center">
        <a class="home" href="<?php echo _SITE_URL;?>"><?php echo _HOME?></a><span> | </span><span><?php echo _NAJAM;?></span>
    </div>
</div> 
...
<div class="row content <?php echo($lokacije_idijevi)?' filter-items':'';?>">
	<div class="center">
		<div class="options">
			<h1><?php echo _NAJAM;?> - <strong><?php echo $main['title_'._LNG];?></strong> (<?php echo $ukupno;?>) </h1>
		</div>	
		<div class="clearfix"></div>
		<div class="filters">
			<form name="filter" action="<?php echo $filter_url; ?>" method="GET">
					...
					<span class="img-frame">	
						...
						<span class="overlay">
							<span class="icon-frame">
								...
								<?php echo ($red['bathrooms']!='')?'<span class="icon"><img src="images/bathrooms.svg" /><span>'.$red['bathrooms'].'</span></span>':'';?>
								<?php echo ($red['rooms2']>0)?'<span class="icon"><img src="images/rooms.svg" /><span>'.$red['rooms2'].'</sup></span></span>':'';?>
							</span>
						</span>
					</span>
...
?>
	<div class="pagination row">
		<div class="center">
			<?php 
			if($page > 1){ 
				echo '<a class="prev" href="'._SITE_URL._LNG.'/'._URL_NAJAM.'/'.$kat_filter.'/'.($page-1).'/'.$nastavak.'">'._PRETHODNA.'</a>';
			} 
			if($stranica > 10){
				...
				for($i=$start_page;$i<=$end_page;$i++){
					echo '<a href="'._SITE_URL._LNG.'/'._URL_NAJAM.'/'.$kat_filter.'/'.$i.'/'.$nastavak.'" '.(($page == $i)? 'class="active"':'').'>'.$i.'</a>';
				}
			}else{
			...
			if($page < $stranica){ 
				echo '<a class="next" href="'._SITE_URL._LNG.'/'._URL_NAJAM.'/'.$kat_filter.'/'.($page+1).'/'.$nastavak.'">'._SLJEDECA.'</a>';
			} 
			?>
		</div>
	</div>
<?php 
 ...
 $bg = '<div class="top"><img class="top-img" src="images/bg.jpg" /><div class="center"><h1>'._NAJAM.'</h1></div></div>';
...
<div class="row content">
	<div class="center txt-center services">
			...
			<div class="items">
				<a href="<?php echo _SITE_URL._LNG.'/'._URL_NAJAM.'/'.clean_uri('Zagreb')?>" class="w30" style="">
                    <span class="img-frame">    
                ...
                <a href="<?php echo _SITE_URL._LNG.'/'._URL_NAJAM.'/'.clean_uri('Jadran')?>" class="w30" style="">
                    <span class="img-frame">    
                ...
                <a href="<?php echo _SITE_URL._LNG.'/'._URL_NAJAM.'/'.clean_uri(_OSTALO)?>" class="w30" style="">
                    <span class="img-frame">    
